...
num_customers = len(pd.read_csv("/Users/griffmw/Documents/PhD/Research/5 Current/GVRP Main Code/Walmart Small/Demand.csv", header=None))-1
...
Zmax = 1  # number of tours or days in the planning horizon
...
#Distance matrix:
Distance_Matrix = pd.read_csv("/Users/griffmw/Documents/PhD/Research/5 Current/GVRP Main Code/Walmart Small/Distances.csv", header=None)
distance_array = Distance_Matrix.values

#Travel time matrix:
Travel_Times = pd.read_csv("/Users/griffmw/Documents/PhD/Research/5 Current/GVRP Main Code/Walmart Small/Times.csv", header=None)
...
Customer_demand = pd.read_csv("/Users/griffmw/Documents/PhD/Research/5 Current/GVRP Main Code/Walmart Small/Demand.csv", header=None)
...
#Upsilon: price per carbon allowance ($USD per metric ton of CO2)
Upsilon = 40

#G: carbon cap for firm (metric tons of CO2 per year)
G = 1000
...
#W: Driver wages per second ($USD/s)
W = 35 / 60/ 60  
...
# -------------------------------------------------------
#    Integer variable.
#    Keys: (k, z, i, j) where i, j ∈ N_expanded (packed tuples)
x = model.addVars(
    ((k, z, i, j) for k in K_list for z in Z for i in N_expanded for j in N_expanded if i != j),
...
q = model.addVars(
    ((k, z, i, j) for k in K_list for z in Z for i in N_expanded for j in N_expanded if i != j),
    vtype=GRB.CONTINUOUS, lb=0.0, name="q"
)
...
#    Keys: (k, z, i)
y = model.addVars(
    ((k, z, i) for k in K_e_list for z in Z for i in N_expanded),
...
U = model.addVars(
    ((k, z, i) for k in K_list for z in Z for i in N_expanded),
    vtype=GRB.CONTINUOUS, lb=0.0, name="U"
)
...
#   Keys: (k, z, j) where j ∈ D_expanded (packed tuple)
delivered = model.addVars(
    ((k, z, j) for k in K_list for z in Z for j in D_expanded),
    vtype=GRB.CONTINUOUS, lb=0.0, name="delivered"
)

# -------------------------------------------------------
# 3.13  e_plus/e_minus: carbon trading variables
# -------------------------------------------------------
# e_plus: amount of carbon allowances purchased (tCO2)
# e_minus: amount of carbon allowances sold (tCO2)
...
# -------------------------------------------------------
term3 = F * gp.quicksum(
    phi_param * sigma_param * lambda_param * d_exp[(i, j)] * q[(k, z, i, j)]
    for k in K_c_list for z in Z
    for i in N_expanded for j in N_expanded if i != j
)

# -------------------------------------------------------
# 1.4 General Operating Costs + DriverSalaries
# -------------------------------------------------------
term4 = gp.quicksum(
    O[k] * d_exp[(i, j)] * x[(k, z, i, j)]
...
# -------------------------------------------------------
# 1.8 Carbon Trading Costs
# -------------------------------------------------------
# Cost incurred or revenue earned based on purchased/sold allowances.
term8 = Upsilon * (e_plus - e_minus)

# -------------------------------------------------------
# Complete Expression
# -------------------------------------------------------
obj_expr_cost = term1 + term2 + term3 + term4 + term5 + term6 + term7 + term8
...
# Complete Expression
# -------------------------------------------------------
obj_expr_emissions = (term9 + term10)
...
# -------------------------------------------------------
model.addConstrs(
    gp.quicksum(x[(k, z, (depot, 0), j)] for z in Z for j in N_expanded if j != (depot, 0)) >= used[k]
    for k in K_list
)
...
model.addConstrs(
    gp.quicksum(x[(k, z, (depot, 0), j)] for j in N_expanded if j != (depot, 0)) ==
    gp.quicksum(x[(k, z, i, (depot, 0))] for i in N_expanded if i != (depot, 0))
    for k in K_list for z in Z
)
...
# -------------------------------------------------------
model.addConstrs(
    (gp.quicksum(x[(k, z, i, j)] for i in N_expanded if i != j) -
     gp.quicksum(x[(k, z, j, i)] for i in N_expanded if i != j)) == 0
...
model.addConstrs(
    (T[(k, z)] <= Tmax for k in K_list for z in Z),
    name="Maximum_service_time_per_day"
)
...
for k in K_list:
    for z in Z:
        for j in N_expanded:
            if j != (depot, 0):
                model.addConstr(used[k] >= x[(k, z, (depot, 0), j)],
...
for k in K_list:
    for z in Z:
        for j in D_expanded:
            model.addConstr(
                gp.quicksum(q[(k, z, i, j)] for i in N_expanded if i != j) ==
                delivered[(k, z, j)] + gp.quicksum(q[(k, z, j, i)] for i in N_expanded if i != j),
        ...
        for i in N_expanded:
            for j in N_expanded:
                ...
                if i == (depot, 0) and j in D_expanded:
                    ...
                    model.addGenConstrIndicator(
                        x[(k, z, i, j)], True,
                        U[(k, z, j)] == U[(k, z, i)] - delivered[(k, z, j)],
...
for k in K_list:
    for z in Z:
        for j in D_expanded:
            model.addConstr(
                delivered[(k, z, j)] <= p[j[0]] * gp.quicksum(x[(k, z, j, i)] for i in N_expanded if i != j),
...
for k in K_list:
    for z in Z:
        for j in D_expanded:
            model.addConstr(
                delivered[(k, z, j)] >= Q[k] / 2 * gp.quicksum(x[(k, z, j, i)] for i in N_expanded if i != j),
...
# =============================================================================
model.addConstr(
    obj_expr_emissions + e_minus <= (G * Zmax / 365.0) + e_plus,
    name="carbon_cap_constraint"
)
...
model.setObjectiveN(obj_expr_cost, index=1, priority=1, name="Minimize_Total_Cost")
model.setObjectiveN(obj_expr_emissions, index=0, priority=0, name="Minimize_GHG_Emissions")
...
carbon_allowance_cost = term8.getValue()
total_cost = (recharging_cost + diesel_cost_curb + diesel_cost_load + operating_cost + acquisition_cost + 
hgev_infra_cost + diesel_emissions_cost + carbon_allowance_cost)

#Emissions
total_diesel_emissions = term9.getValue()
hgev_emissions = term10.getValue()

# -------------------------------------------------------
# Arc and vehicle selection
# -------------------------------------------------------
# x is now indexed as (k, z, i, j) where i, j ∈ N_expanded (packed tuples)
selected_arcs = [
    (k, z, i, j)
    for (k, z, i, j) in x.keys()
    if x[(k, z, i, j)].X > 0.5
]
...
print("Carbon allowance cost:", carbon_allowance_cost)
...
# -------------------------------------------------------
# Carbon Trading Results
# -------------------------------------------------------
print("\n--- Carbon Trading Results ---")

e_plus_val = e_plus.X
e_minus_val = e_minus.X

# Compute the net trading position (positive = net purchase, negative = net sale)
net_credits = e_plus_val - e_minus_val

print(f"Carbon allowances purchased (e+): {e_plus_val:.4f} metric tons CO2")
print(f"Carbon allowances sold (e-): {e_minus_val:.4f} metric tons CO2")
print(f"Net trading position (e+ - e-): {net_credits:.4f} metric tons CO2")

# Compute total carbon trading cost/revenue
carbon_trading_cost = Upsilon * net_credits
print(f"Carbon trading cost (Υ·(e+ - e-)): ${carbon_trading_cost:,.2f}")

# -------------------------------------------------------
# Partial delivery verification
# -------------------------------------------------------
print("\nVerifying Vehicle Load Reduction After Partial Deliveries:")
for k in K_list:
    for z in Z:
        print(f"\nVehicle {k}, Tour {z}:")
        # Find all arcs selected by this vehicle in tour z: x is now indexed as (k, z, i, j)
        selected_arcs_kz = [
            (i, j) 
            for i in N_expanded 
            for j in N_expanded 
            if i != j and x[(k, z, i, j)].X > 0.5
        ]
        if not selected_arcs_kz:
            print("  No arcs selected.")
            continue
        # For each selected arc, check the cumulative load update.
        for i, j in selected_arcs_kz:
            if j == (depot, 0):
                # When arriving at the depot, the load resets to full capacity.
                print(f"  Arc ({i} -> {j}): Arriving at depot. U[{k}, Tour {z}, {j}] = {U[(k, z, j)].X:.2f} (should be Q[{k}] = {Q[k]:.2f}).")
            elif i == (depot, 0) and j != (depot, 0):
                # When leaving the depot, expected load at j = full capacity minus the delivery at j.
                expected_load = Q[k] - delivered[(k, z, j)].X
                print(f"  Arc ({i} -> {j}): U[{k}, Tour {z}, {j}] = {U[(k, z, j)].X:.2f}, Delivered[{k}, Tour {z}, {j}] = {delivered[(k, z, j)].X:.2f}, Expected U[{k}, Tour {z}, {j}] = {expected_load:.2f}")
            elif i != (depot, 0) and j != (depot, 0):
                # For arcs between customers: expected load at j = load at i minus delivery at j.
                expected_load = U[(k, z, i)].X - delivered[(k, z, j)].X
                print(f"  Arc ({i} -> {j}): U[{k}, Tour {z}, {i}] = {U[(k, z, i)].X:.2f}, Delivered[{k}, Tour {z}, {j}] = {delivered[(k, z, j)].X:.2f}, U[{k}, Tour {z}, {j}] = {U[(k, z, j)].X:.2f}, Expected U[{k}, Tour {z}, {j}] = {expected_load:.2f}")
...
for k in K_e_list:
    ...
    for z in Z:
        ...
        # Note: x is now indexed as (k, z, i, j)
        vehicle_arcs = [
            (i, j) 
            for i in N_expanded 
            for j in N_expanded 
        ...
        for tour in tours:
            ...
            for node in tour:
                # Access y using the packed key: (k, z, node)
                battery_level = y[(k, z, node)].X
                if node == (depot, 0):
...
for k in K_list:
    ...
    for z in Z:
        ...
        # Note: x is defined over (k, z, i, j)
        vehicle_arcs = [
            (i, j)
            for i in N_expanded 
            for j in N_expanded 
